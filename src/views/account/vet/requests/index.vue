      ...
      <template v-if="isLoad">
        <b-row v-if="displayMode === 'image'" key="image">
          ...
          <!--
            ...
            <req-thumb-list-item @refresh="loadItems" :key="item.id" :data="item" v-contextmenu:contextmenu />
          </b-colxx>
        </b-row>
...
import ReqThumbListItem from "@/components/Listing/ReqThumbListItem";
...
export default {
  components: {
    ...
    "req-thumb-list-item": ReqThumbListItem
  },
  data() {
    return {
      file1: null,
  ...
  methods: {
    loadItems() {
      ...
      this.items = []
      this.$Axios.get('/dossierRequests')
       .then(res => {
